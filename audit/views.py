...
def home(request):
    ...
    # Get audit findings statistics from the Patient model
...
def audit_form(request):
    if request.method == 'POST':
        try:
            ...
            messages.error(request, f'Error creating audit record: {str(e)}')
    ...
    try:
        ...
        for audit in audits:
            audit_data = {
                ...
                'District': audit.district.name,
                'Hospital Type': audit.ehcp_type,
                'Visit Date': audit.visit_date.strftime('%Y-%m-%d'),
                'Visit Time': audit.visit_time.strftime('%H:%M:%S') if audit.visit_time else '',
                'Auditor Name': audit.auditor_name,
                'Designation': audit.designation,
                'Location': audit.current_location,
                'EKGP Patients': audit.ekgp_patients,
                'PMJAY Patients': audit.pmjay_patients,
                'Total Beneficiaries': audit.beneficiaries,
                'Findings Type': audit.findings_type,
                'Audit Findings Value': audit.audit_findings_value,
                'Finding Type': audit.finding_type,
                'Abuse Type': audit.abuse_type,
                'OOPE Type': audit.oope_type,
                'HNQA Value': audit.hnqa_value,
                'HNQA Type': audit.hnqa_type,
                'Infrastructure Type': audit.infrastructure_type,
                'HR Type': audit.hr_type,
                'Services Type': audit.services_type,
                'Fraudulent Value': audit.fraudulent_value,
                'Fraudulent Type': audit.fraudulent_type,
            ...
            for patient in patients:
                patient_data = {
                    ...
                    'Discharge Date': patient.discharge_date.strftime('%Y-%m-%d') if patient.discharge_date else '',
                    ...
                    'Case Summary': patient.case_summary,
                    'Deviations': ', '.join(patient.deviations) if patient.deviations else '',
                    'Total OOPE': patient.total_oope,
                    'Patient Photo': patient.patient_photo.url if patient.patient_photo else '',
                    'Case File': patient.case_file.url if patient.case_file else '',
                    'Discharge Summary': patient.discharge_summary.url if patient.discharge_summary else '',
                    'Bills & Documents': patient.bills_documents.url if patient.bills_documents else ''
                }
                all_data.append({**audit_data, **patient_data})
        
        df = pd.DataFrame(all_data)
        response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response['Content-Disposition'] = f'attachment; filename="all_data_{datetime.now().strftime("%Y%m%d")}.xlsx"'
        with pd.ExcelWriter(response, engine='openpyxl') as writer:
            df.to_excel(writer, index=False, sheet_name='All Data')
        return response
    except Exception as e:
        return HttpResponseBadRequest(f'Error generating Excel report: {str(e)}')
